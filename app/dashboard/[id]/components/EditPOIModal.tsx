...
import { MapPin, X, Camera } from "@/icons/Icons";
...
export function EditPOIModal({ isOpen, onClose, poi }: EditPOIModalProps) {
  ...
  useEffect(() => {
    if (poi && isOpen) {
      setPoiName(poi.title);
      setFacility(poi.categoryType);
      setBuilding(poi.building);
      setFloor(poi.floor);
      setRoomNumber(poi.roomNumber);
      setDescription(poi.description);
      setTags(poi.categories.join(", "));
      setWheelchairAccessible(poi.accessibility?.wheelchair || false);
      setHearingLoop(poi.accessibility?.hearingLoop || false);
      setVisualAidSupport(poi.accessibility?.visualAidSupport || false);
      setEmail("");
      setPhone("");
      setOperatingHours("");
      setRoomNumberAlt(poi.roomNumber);
      setXCoordinate("0");
      setYCoordinate("0");
    }
  }, [poi, isOpen]);
