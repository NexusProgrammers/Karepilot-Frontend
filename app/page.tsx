...
export default function Home() {
  ...
  return (
    <>
      {!loggedIn ? (
        <KarepilotLogin onLogin={() => setLoggedIn(true)} />
      ) : (
        <DashboardLayout>
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-4">
              {organizations.map((org, index) => (
                <div
                  key={index}
                  className="bg-white rounded-3xl border border-gray-200 p-10 hover:border-[#3D8C6C] transition-shadow"
                >
                  ...
                  <Button className="w-full bg-blue-100 hover:bg-[#3B73B5] text-[#3B73B5] hover:text-white cursor-pointer">
                    Select
                  </Button>
                </div>
              ))}
